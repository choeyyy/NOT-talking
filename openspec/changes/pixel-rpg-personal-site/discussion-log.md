...
4. **唤醒：** **地精** **不**随主程序常醒 — 造物主 **打开地精聊天窗** 后才调用（见 §N）。**长身人运维** 与 **之子** **常启**。
...
| ~~B8~~ | API 成本 / 唤醒 | **已决**：地精 **不**后台常醒 — 仅造物主 **点开** 地精聊天窗后唤醒；关窗 dormant。**长身人运维** 与 **之子** 同为 **常启**（随主程序启动，关窗只关 UI session） |
...
1. **角色：** 创世主旗下的 **长身人团队**（与地精对称）— 修世界相关 **机制/代码问题**；冒险者可见 **长身人代码公司** 门面、**被拒入**
2. **大世界拒入文案（拍板）：** **「非法闯入请刷卡。」** — 静态 JRPG 对话框，不调用 AI
3. **地精能热更的：** 仅 **数据**（`dungeon_scripts` 等 DB + cache invalidate）— **不是**改程序
...
| ID | 问题 |
|----|------|
| ~~M1~~ | ~~长身人入口~~ → **已决**：大世界 `tall_folk_guild_entrance` + 后台 `/admin/tall-folk`；拒入 **「非法闯入请刷卡。」** |
...
| **造物主登录** | 看见与冒险者 **同一套** 主要世界与玩家可见页面（`/world`、`/home`…），另加 `/admin`；**不**因登录自动拉起地精聊天 |
| **地精** | **休眠** 直到造物主 **打开** 地精协会聊天窗 → 唤醒；关窗 → log + flush → dormant |
| **长身人运维** | **常启** — 随主程序启动；可后台读 log / 开 issue；造物主开 `/admin/tall-folk` **无需唤醒**；关窗只结束 UI session |
| **关窗（地精）** | 聊天面板关闭 → **先** 聊天记录 + 草案快照落服务器 log → **再** session 结束 → 地精 dormant |
| **之子** | **常启** — 随主程序启动，冒险者随时可聊；**不**唤醒地精 |
| **神谕** | 登录用户 **点浮层** 才请求（按需） |
...
```
[常跑] 冒险者 /world、副本 API、之子、长身人运维（后台 diagnostic）
         │
         ▼
...
## P. 美术资源库 · 线上标号 · 玩家资产入库（2026-08-11 · 用户拍板）
...
|----|------|
| **入口** | **`/admin/assets`** |
| **范围** | **全局**（场景/NPC/道具模板）+ **玩家**（`/profile` 角色、`/workshop` 造物 sprite） |
| **标号** | 造物主可设 **`asset_number`**（目录编号），可搜索 |
| **改细节** | label、tags、notes、kind 等 **线上编辑**，T0 热更（DB + invalidate） |
| **换图** | 可替换 PNG，`version++`，依赖方下次加载刷新 |
| **关联** | NPC / 场景 / studio 选用资源时走 **`art_asset_id`** |

**相关 spec：** `admin-assets`

---

## Q. 长身人运维常启（2026-08-11 · 用户拍板）

### 已定

| 项 | 口径 |
|----|------|
| **与之子同级** | **长身人运维**（三地精式 binding 组成的修会后端）随主程序 **常启**，**不休眠** |
| **地精仍按需** | 地精协会 **仅** 造物主打开聊天窗后唤醒；关窗 dormant |
| **关窗** | 长身人关 `/admin/tall-folk` → 只关 **UI session** + 落 log；运维 binding **仍常驻** |
| **后台** | 可离线读 log / 开 issue（受 `agent-safety` 限额） |

**相关 spec：** `tall-folk-guild`, `agent-life`, `creator-son-agent`

---

## R. 自选 Agent 拉群 — 不做（2026-08-11 · 用户拍板）

### 已定

**不实现** 造物主自选 Agent/NPC 任意组群的聊天室。

| 原因 | 说明 |
|------|------|
| **上下文乱** | 多 binding 同 thread，system prompt / 人设 / 工具白名单易串 |
| **唤醒乱** | 地精 dormant、长身人常启、之子常启 — 拉进一群后谁算醒、谁可调 API 难定 |
| **路由乱** | @ 谁、轮询谁、多 Agent 同时回，体验和 audit 都差 |

**保留：** 固定 **地精协会群**、**长身人修会群** + 各 **1:1**（之子、神谕、赋活造物、玩家 DM）。静态 NPC 仍走台词，不进 AI 群。

**相关 spec：** `agent-life`, `gnome-guild`, `tall-folk-guild`

---

## S. AI 模型 · Token 成本 · 玩家扩容（2026-08-11 · 讨论记录）

### 已定方向

| 项 | 口径 |
|----|------|
| **不自训模型** | V1 **不需要** 训练/托管自有 LLM；**Cursor API + `MODEL_MAP`**（Composer 2.5 / Grok 4.5 等）+ **`system_prompt` / `agent_bindings`** 即可 |
| **Persona ≠ 微调** | 之子、地精、长身人、赋活造物 = **同一或不同商用 model + 不同 prompt + AgentRouter** |
| **Fine-tune** | **非 V1 门禁**；仅当 prompt/RAG 仍不够且 **有大量高质量专有语料** 时，再考虑 **窄场景**（如地精 JSON）；**不降低** 推理 token 单价 |
| **玩家侧 AI 入口** | V1 主要 **之子 + 神谕**；大世界 **静态 NPC 默认**；赋命 NPC / 赋活造物 V2+ **限量** |
| **扩容手段** | **限流**（已有草案）· **静态台词** · **模型分层**（便宜 model 给日常）· **`/admin/agents` kill switch / 日预算** · 高峰队列/降级 · 可选玩法配额（AP/金币） |
| **Grok 等「别人的模型」** | 经 Cursor 代理同样 **按 token 计费**；切换 model 不 magically 省钱，贵不贵看单价 + 调用量 + 回复长度 |

### 待定（实现前可调）

| ID | 问题 |
|----|------|
| S1 | `/admin/agents` 是否暴露 **日总 token 预算** 与 **按 Agent 类默认 model** |
| S2 | 赋命世界 NPC 对玩家开放 AI 时，是否 **单独更低** 的 per-user 限额 |
| S3 | 之子繁忙 **静态 fallback** 文案是否 V1 就要 |

**相关 spec：** `agent-safety`, `ai-oracle`, `creator-son-agent`, `agent-life`, `design.md` §10
